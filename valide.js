/* Harnais de validation — Blackhawks de Chicago (jsdom) */
const fs = require('fs');
...
const ATTENDU = {
  equipe: 'CHICAGO',
  taille: 22,
  echantillon: {nom: 'Miro Heiskanen', salaire: 9075000, ct: 1, ov: 82},
  masseSousContrat: 36675000,
  nbSousContrat: 8
};
...
(async () => {
  ...
  const vKan = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Patrick Kane'), 'UFA', 1);
  egal(vKan.code, 'surpaye', 'Patrick Kane (OV 76, 38 ans, 5,125 M, UFA) → Surpayé');
  egal(vKan.minEff, 1000000, 'Charte Kane : 1 000 000 $ (ligne 35 et +)');
  egal(vKan.ecart, 4125000, 'Écart Kane : +4 125 000 $');
  const vRos = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Isak Rosen'), 'RFA', 1);
  egal(vRos.code, 'aubaine', 'Isak Rosen (OV 77, 23 ans, 900 k, RFA) → Aubaine salariale');
  egal(vRos.ecart, 50000, 'Écart Rosen : −50 000 $ sous la charte (950 k)');
  ...
  const vGus = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Filip Gustavsson'), 'RFA', 1);
  egal(vGus.minEff, 3750000, 'Filip Gustavsson (G, OV 80, 28 ans, RFA) évalué à l\'échelon 79 : 3 750 000 $');
  egal(vGus.code, 'aubaine', 'Filip Gustavsson à 3,125 M → Aubaine salariale');
  ...
  selJoueur.value = 'Patrick Kane';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Kane sélectionné : badge Surpayé');
  ...
  egal(celluleActive.textContent.replace(/[\s\u00a0\u202f]/g,''), '1000$',
    'Cellule active : 1 000 $ (UFA 35+, OV 76, Kane)');
  egal(doc.getElementById('csStatut').value, 'UFA', 'Charte UFA proposée d\'après l\'âge (38 ans)');
  ...
  selJoueur.value = 'Filip Gustavsson';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('aubaine'), 'Filip Gustavsson : badge Aubaine salariale');
